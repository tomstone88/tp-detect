 ...
 */
async function scanEC2Instances(clientConfig, options) {
    const { logger } = options;
    const results = [];
    const ec2Client = new client_ec2_1.EC2Client(clientConfig);
    // Get all EC2 instances
    const listCommand = new client_ec2_1.DescribeInstancesCommand({});
    ...
    for (const reservation of response.Reservations) {
        ...
        for (const instance of reservation.Instances) {
            ...
            if (instance.UserData) {
                ...
                const matches = (0, content_scanner_1.scanContent)(userData, options);
                if (matches.length > 0) {
            ...
            if (instance.Tags && instance.Tags.length > 0) {
                ...
                // Scan tags content
                const matches = (0, content_scanner_1.scanContent)(tagsContent, options);
                if (matches.length > 0) {
                    const score = calculateScore(matches);
                    results.push({
                        timestamp: new Date().toISOString(),
    ...
    try {
        const templatesCommand = new client_ec2_1.DescribeLaunchTemplatesCommand({});
        ...
        if (templatesResponse.LaunchTemplates) {
            for (const template of templatesResponse.LaunchTemplates) {
                ...
                const dataCommand = new client_ec2_1.GetLaunchTemplateDataCommand({
                ...
                if (dataResponse.LaunchTemplateData) {
                    ...
                    // Scan template data
                    const matches = (0, content_scanner_1.scanContent)(templateData, options);
                    if (matches.length > 0) {
                        const score = calculateScore(matches);
                        results.push({
                            timestamp: new Date().toISOString(),
 ...
 */
async function scanLambdaFunctions(clientConfig, options) {
    const { logger } = options;
    const results = [];
    ...
    for (const func of response.Functions) {
        ...
        const getCommand = new client_lambda_1.GetFunctionCommand({
        ...
        if (funcDetails.Code?.Location) {
            logger.debug(`Lambda function ${functionName} has downloadable code, but downloading and scanning code is not implemented yet.`);
 ...
 */
async function scanCloudFormationStacks(clientConfig, options) {
    const { logger } = options;
    const results = [];
    const cfnClient = new client_cloudformation_1.CloudFormationClient(clientConfig);
    ...
    for (const stack of response.StackSummaries) {
        ...
        // Get stack template
        const getCommand = new client_cloudformation_1.GetTemplateCommand({
            StackName: stackName,
        });
        try {
            ...
            if (templateResponse.TemplateBody) {
                // Scan template body
                const matches = (0, content_scanner_1.scanContent)(templateResponse.TemplateBody, options);
                if (matches.length > 0) {
                    const score = calculateScore(matches);
                    results.push({
                        timestamp: new Date().toISOString(),
                        ...
                        contentPreview: templateResponse.TemplateBody.substring(0, 200) +
 ...
 */
async function scanECSTasks(clientConfig, options) {
    const { logger } = options;
    const results = [];
    const ecsClient = new client_ecs_1.ECSClient(clientConfig);
    // Get all task definitions
    const listCommand = new client_ecs_1.ListTaskDefinitionsCommand({});
    ...
    for (const taskDefArn of response.taskDefinitionArns) {
        ...
        const getCommand = new client_ecs_1.DescribeTaskDefinitionCommand({
        ...
        try {
            ...
            if (taskDefResponse.taskDefinition) {
                ...
                const matches = (0, content_scanner_1.scanContent)(taskDefJson, options);
                if (matches.length > 0) {
                    const score = calculateScore(matches);
                    results.push({
                        timestamp: new Date().toISOString(),
 ...
 */
async function scanSSMDocuments(clientConfig, options) {
    const { logger } = options;
    const results = [];
    const ssmClient = new client_ssm_1.SSMClient(clientConfig);
    // Get all SSM documents
    const listCommand = new client_ssm_1.ListDocumentsCommand({
        DocumentFilterList: [
            {
                key: "Owner",
                value: "Self",
    ...
    for (const doc of response.DocumentIdentifiers) {
        ...
        const getCommand = new client_ssm_1.GetDocumentCommand({
            Name: docName,
        });
        try {
            ...
            if (docResponse.Content) {
                ...
                const matches = (0, content_scanner_1.scanContent)(docResponse.Content, options);
                if (matches.length > 0) {
                    const score = calculateScore(matches);
                    results.push({
                        timestamp: new Date().toISOString(),
