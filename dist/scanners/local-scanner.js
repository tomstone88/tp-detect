...
exports.scanLocalSystem = scanLocalSystem;
...
const base64_detector_1 = require("../utils/base64-detector");
const execAsync = (0, util_1.promisify)(child_process_1.exec);
/**
 * Scans the local system for cryptojacking indicators
 */
async function scanLocalSystem(options) {
    const results = [];
 ...
 */
async function scanLinuxStartupScripts(options, results) {
    const { logger } = options;
    ...
    try {
        ...
        const userStartupFiles = [
            ...
            path_1.default.join(homeDir, ".cshrc"),
        ];
    ...
    for (const location of startupLocations) {
        try {
            const stats = await promises_1.default.stat(location);
 ...
 */
async function scanLinuxCronJobs(options, results) {
    const { logger } = options;
 ...
 */
async function scanLinuxServices(options, results) {
    const { logger } = options;
    ...
    try {
        const { stdout } = await execAsync("systemctl list-units --type=service --state=running --no-pager --plain --no-legend");
 ...
 */
async function scanWindowsStartupLocations(options, results) {
    const { logger } = options;
    // Common startup locations
    const startupLocations = [
        path_1.default.join(os_1.default.homedir(), "AppData", "Roaming", "Microsoft", "Windows", "Start Menu", "Programs", "Startup"),
        path_1.default.join("C:", "ProgramData", "Microsoft", "Windows", "Start Menu", "Programs", "StartUp"),
        path_1.default.join("C:", "Windows", "System32", "GroupPolicy", "Machine", "Scripts", "Startup"),
        path_1.default.join("C:", "Windows", "System32", "GroupPolicy", "User", "Scripts", "Logon"),
    ];
    // Scan each location
    for (const location of startupLocations) {
        try {
    ...
    try {
        const { stdout: runKeys } = await execAsync('reg query "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" /s');
        if (runKeys) {
 ...
 */
async function scanWindowsScheduledTasks(options, results) {
    const { logger } = options;
    try {
 ...
 */
async function scanWindowsServices(options, results) {
    const { logger } = options;
    try {
        // List all services
