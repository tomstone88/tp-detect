 ...
 */
export async function scanGCP(options: ScanOptions): Promise<ScanResult[]> {
 ...
 */
async function scanComputeInstances(projectId: string | undefined, options: ScanOptions): Promise<ScanResult[]> {
    ...
    for (const instance of instances) {
      ...
      if (metadata.items) {
        ...
        if (startupScript && startupScript.value) {
          // Scan startup script
          const matches = scanContent(startupScript.value, options)
      ...
      // Scan metadata
      const metadataMatches = scanContent(metadataJson, options)
  ...
  try {
    ...
    for (const template of templates) {
      ...
      // Scan template
      const matches = scanContent(templateJson, options)
 ...
 */
async function scanCloudFunctions(projectId: string | undefined, options: ScanOptions): Promise<ScanResult[]> {
  const { logger } = options
  const results: ScanResult[] = []
