 ...
 */
export async function scanAzure(options: ScanOptions): Promise<ScanResult[]> {
 ...
 */
async function scanVirtualMachines(
  ...
  for (const vm of vms) {
      ...
      for (const extension of extensions) {
        ...
        if (
          ...
          if (extension.settings) {
            ...
            // Scan settings
            const matches = scanContent(settingsJson, options)
    ...
    if (vm.osProfile) {
      ...
      if (vm.osProfile.customData) {
        ...
        // Scan custom data
        const matches = scanContent(customData, options)
      ...
      if (vm.osProfile.linuxConfiguration?.provisionVMAgent) {
        ...
        if (vm.osProfile.linuxConfiguration.ssh?.publicKeys) {
          ...
          // Scan SSH keys
          const matches = scanContent(sshKeys, options)
 ...
 */
async function scanVMScaleSets(
  subscriptionId: string,
  credentials: DefaultAzureCredential,
  options: ScanOptions,
): Promise<ScanResult[]> {
 ...
 */
async function scanContainerInstances(
  subscriptionId: string,
  credentials: DefaultAzureCredential,
  options: ScanOptions,
): Promise<ScanResult[]> {
  ...
  const containerClient = new ContainerInstanceManagementClient(credentials, subscriptionId)
    ...
    if (group.containers) {
      for (const container of group.containers) {
        ...
        if (container.command) {
          ...
          // Scan command line
          const matches = scanContent(commandLine, options)
 ...
 */
async function scanResourceGroups(
  subscriptionId: string,
  credentials: DefaultAzureCredential,
  options: ScanOptions,
): Promise<ScanResult[]> {
    ...
    for (const resource of resources) {
      ...
      try {
        ...
        if (resourceDetails.properties) {
          ...
          // Scan properties
          const matches = scanContent(propertiesJson, options)
